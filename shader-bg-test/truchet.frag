...
// Shadertoy 4td3zj (Raymarched Hexagonal Truchet) rendered by quickshell on a wl_surface.
// Original by Shane: https://www.shadertoy.com/view/4td3zj
//
// Adapted for Qt Quick's ShaderEffect:
//   - iTime / iResolution / iMouse arrive through the default uniform block instead of
//     being declared as standalone uniforms, in the order the QML properties are declared.
//   - iResolution is vec2 rather than vec3; the shader body only ever uses .xy / .x / .y.
//   - mainImage() is called from main(), with shadertoy's bottom-left fragCoord origin
//     reconstructed from Qt's top-left qt_TexCoord0.
//
// waves.frag is the previous wallpaper and is kept around as an alternative; point
// shell.qml's sourcePath at it to switch back.
...
// Wallpaper pacing knob. 1.0 is the original shadertoy speed; lower values slow the
// whole scene down together (plane rotation, camera drift, light orbit, Voronoi pulses
// and the environment-noise scroll).
...
void main() {
    vec4 col;
    // Shadertoy fragCoord is measured in pixels with the origin at the bottom-left;
    // Qt's qt_TexCoord0 is normalised with the origin at the top-left.
