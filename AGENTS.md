...
| `src/render_helpers/cursor_effect.rs` | `CursorEffect`, `CursorEffectGeometry`, `cursor_effect_geometry()`, `cursor_frame_identity()` — renders the cursor as a padded `FramebufferEffectElement` through the shader pipeline. |
...
| `niri-config/src/misc.rs` | `Cursor`/`CursorPart` — xcursor theme/size, `effect-padding`, and the cursor `shader_pipeline` |
...
- `mask_coverage.frag` — writes cursor alpha into the bbox-local binary coverage texture; the seed for `cursor-vectors`
...
- **Cursor mask:** the `cursor-vectors` JFA cache additionally keys on `cursor_frame_identity()` (icon, scale, animation frame, size). Cursor motion changes neither the silhouette identity nor its placement in the padded mask, so moving the cursor only re-blits the cached encoded mask; JFA+Poisson reruns merely on animation frames, which are recomputed per frame for now (caching per animation frame is a possible optimization).
- **Cursor coverage textures:** `CursorTextureCache` imports each named cursor frame's premultiplied ARGB into a `GlesTexture` once per (icon, scale, frame), cleared on cursor config reload.
...
- **Shader pipeline examples:** `shaders/crt/`, `shaders/magnify/`, `shaders/overshifted3/`, `shaders/cursor-glass/`, `shaders/debug-mask/`, `shaders/debug-edges/`, `shaders/jfa-debug/`
