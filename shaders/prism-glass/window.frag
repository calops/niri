...
const float bevel_width = 104.0;
const float refraction_px = 28.0;
const float chromatic_px = 1.35;
...
void main() {
    ...
    // A softened circular arc controls the surface normal. Its angles sweep
    // naturally from a steep rim to a horizontal plateau. Refraction uses a
    // separate smooth profile below so optical strength cannot fold the image.
    ...
    // Surface slope is independent from displacement. The multiplier restores
    // the steepness lost by regularizing the circular rim singularity.
    ...
    // Beer-Lambert transmission through the longer path at the curved edge.
    // A nearly neutral body tint gives the slab density without warming the
    // scene behind it.
    ...
    // the front surface. Their widths are expressed in bevel coordinates, not
    // pixels, so they stay coherent with the refraction profile.
    ...
    float outer_glint = 1.0 - smoothstep(0.6, 2.4, inside);
